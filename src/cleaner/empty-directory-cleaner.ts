...
export class EmptyDirectoryCleaner implements IEmptyDirectoryCleaner {
  ...
  /**
   * Sorts path strings by descending depth.
   * @param leftDirectory - Left directory path.
   * @param rightDirectory - Right directory path.
   * @returns Negative when left should come first.
   */
  private comparePathDepthDescending(leftDirectory: string, rightDirectory: string): number {
    return this.getPathDepth(rightDirectory) - this.getPathDepth(leftDirectory);
  }
   ...
   * @returns Updated sorted directories.
   */
  private insertDirectoryByDepth(
    ...
    /**
     * Returns whether the new directory should be inserted before an existing directory.
     * @param existingDirectory - Existing sorted directory.
     * @returns True when the new directory should be inserted first.
     */
    const shouldInsertBefore = (existingDirectory: string): boolean => {
      return this.comparePathDepthDescending(directory, existingDirectory) < 0;
    };

    const insertionIndex = directories.findIndex(shouldInsertBefore);

    return insertionIndex === -1
      ? [...directories, directory]
      : [...directories.slice(0, insertionIndex), directory, ...directories.slice(insertionIndex)];
  }

  /**
   * Returns the segment depth for a directory path.
   * @param directory - Directory path to inspect.
   * @returns Path depth.
   */
